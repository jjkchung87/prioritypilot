...
t2 = Team(name="Finance")
t3 = Team(name="R&D")
t4 = Team(name="Operations")
t5 = Team(name="Supply_chain")
t6 = Team(name="Senior_leadership")
t7 = Team(name="Marketing")

db.session.add_all([t1, t2, t3, t4, t5, t6, t7])
db.session.commit()

teams = (t2, t3, t4, t5, t6, t7)
...
for i in range(6):
    ...
    email = f"{first_name}_{last_name}@company.com"
    role = f"{teams[i].name}_Manager"
    team_id = teams[i].id
    password = f"Password{i}!!!"

    user = User.signup(email, first_name, last_name, team_id, role, password)
